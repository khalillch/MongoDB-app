...
import program_buyers as hosts
import services.data_service as svc
from program_buyers import success_msg, error_msg
...
def surrender_dog():
    # todo: see number of cages first
    account = state.active_account
    if not account:
        error_msg("ERROR: You must login first")
        return
    name = input("give your dog's name: ")
    size = input("give your dog's size: ")
    type = input("give your dog's type: ")
    cage = svc.find_cage(size)
    if not cage:
        error_msg("ERROR: No cage available for now !!")
        return
    dog = svc.add_dog(name, size, type)
    svc.take_cage(cage, dog.id)
    svc.add_surrender(account.id, dog.id)
    success_msg(f"Added new dog with id {dog.id} to cage {cage.id}")
    success_msg(f"Abandoned by customer with id {account.id}")
