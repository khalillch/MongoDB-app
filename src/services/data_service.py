...
def create_account(name: str, email: str) -> Customer:
    customer = Customer()
    customer.name = name
...
def find_dogs() -> Dog:
    ...
    for obj in Dog.objects():
        dogs.append((obj.name, obj.size, obj.type))
    return dogs


def add_dog(name, size, type):
    dog = Dog()
    dog.name = name
    dog.size = size
    if type != "":
...
def add_surrender(account_id, dog_id):
    surrender = DogSurrendering()
    surrender.prev_owner_id = account_id
    surrender.dog_ids.append(dog_id)
    account = Customer.objects(id=account_id).first()
    account.surrendering.append(surrender)
    account.save()
    return surrender


def find_dog(name, size):
    return Dog.objects(name=name, size=size).first()


def delete_dog(name, size):
    dog = find_dog(name, size)
    cage = Cage.objects(dog_id=dog.id).first()
    cage.is_used = False
    cage.save()
    dog.delete()


def add_dog_to_customer(account_id, name):
    customer = Customer.objects(id=account_id).first()
    customer.dog_names.append(name)
    customer.save()


def create_cage(size):
    cage = Cage()
    cage.size = size
    cage.save()
    return cage
...
def find_cages():
    cages = []
    for obj in Cage.objects(is_used=False):
        cages.append((obj.id, obj.size))
    return cages


def find_cage(size):
    return Cage.objects(size=size, is_used=False).first()


def take_cage(cage, dog_id):
    cage.is_used = True
    cage.dog_id = dog_id
    cage.save()
